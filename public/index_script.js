...
$(document).ready(function(){
    ...
    var timeoutID = setInterval(function(){
          // 入料暫存區
          for(let i=1;i<=9;i++)
              InfoBelow('#light'+i, ['#text'+i], '#choose'+i, 'FeedingArea',['重量'],['重量(噸)'],[0,5],[-15,30],'12')
          
          // 粗選機
          for(let i=1;i<=6;i++)
              InfoBelow('#rm_light'+i, ['#rm_text'+i+'_1','#rm_text'+i+'_2'],'#rm'+i, 'RoughingMachine',['重量','雜質'],['重量(噸)','雜質量'],[10,5],[0,30],'10')

           // 流量計
           for(let i=1;i<=4;i++)
              InfoBelow('#fm_light'+i, ['#fm_text'+i],'#fm'+i, 'FlowMeter', ['流量'], ['流量'], [2,0],[-15,20],'10')
           
           // 烘乾暫存區
           for(let i=1;i<=8;i++)
              InfoBelow('#pd_light'+i, ['#pd_text'+i+'_1','#pd_text'+i+'_2'],'#pd'+i, 'PreDryingArea', ['重量','濕度'], ['重量(噸)','濕度'], [0,0],[-10,20],'10')

           // 烘乾機
           for(let i=1;i<=14;i++)
              InfoBelow('#dry_light'+i, ['#dry_text'+i+'_1','#dry_text'+i+'_2','#dry_text'+i+'_3'],'#dry'+i, 'Dryer', ['重量','溫度','濕度'], ['重量(噸)','溫度','濕度'], [10,0],[-10,20],'10')

            // 粗糠桶
           for(let i=1;i<=6;i++)
           InfoBelow('#ca_light'+i, ['#ca_text'+i],'#ca'+i, 'Dryer', ['重量'], ['重量(噸)'], [21,0],[5,20],'10')
          }, 500)
